(function() {
    angular.module('PaQuery')
        .controller('AdminViewPackageController', AdminViewPackageController);

    AdminViewPackageController.$inject = ['$scope', '$state', 'PackagesService', '$uibModal', 'PackageUpdatePosition', 'ProfileService', 'UserService', 'DriverService', 'W3wSerice', '$window', '$filter', 'serverErrorsNotifier', 'DistributionZoneService'];

    function AdminViewPackageController($scope, $state, PackagesService, $uibModal, PackageUpdatePosition, ProfileService, UserService, DriverService, W3wSerice, $window, $filter, serverErrorsNotifier, distributionZoneService) {
        var vm = this,
        ...
        angular.extend(vm, {
            ...
            destiny: {
                ...
                distributionZone: paramPackage && paramPackage.shippingScheduleDestination ? paramPackage.shippingScheduleDestination.distributionZone : null,
            },
            origin: {
                addressInformation: {
        ...
        function init() {
            ...
            if (vm.package) {
                ...
                if(vm.package.shippingScheduleDestination) { //2018-06-06T13:43:06.967Z"
                ...
                if (vm.package.status === 20)
                {
                    getPackageTraceRoute();
                }
        ...
        function getPackageTraceRoute(){
            PackagesService.getPackageTraceRoute(vm.package.id).then(function (response) {
                vm.waypoints = response.data;
        ...
        function getPosiblePackagesStatus() {
            PackagesService.getPosiblePackagesStatus(vm.package.id)
        ...
        function goBack() {
            if (vm.previusUrl=='admin.driver')
                return $state.go(vm.previusUrl, { driver: vm.package.driver });
        ...
        function submit() {
            ...
            // Zona de Distribucion de Destino
            vm.package.shippingScheduleDestination.distributionZone = vm.destiny.distributionZone;
            ...
            PackagesService.updatePackageFromAdminV2( {
        ...
        function onChangeAddress(source) {

            const addressInformation = vm[source].addressInformation;

            const address = addressInformation.address;
            const postalCode = addressInformation.location && addressInformation.location.address && addressInformation.location.address.postalCode;

            distributionZoneService.findDistributionZoneByAddress(address, postalCode)
                .then(function(resp) {
                    if (resp && resp.data) {
                        // console.log("Sen encontro Distribution Zone:" , resp.data)
                        vm[source].distributionZone = resp.data;
                    }
                    else
                        vm[source].distributionZone = undefined;
                })
                .catch(function() {
                    vm[source].distributionZone = undefined;
                })
