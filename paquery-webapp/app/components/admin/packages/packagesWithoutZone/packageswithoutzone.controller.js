(function() {
    ...
    angular
        .module('PaQuery')
        .controller('AdminPackagesWithoutZoneController', AdminPackagesWithoutZoneController);

    AdminPackagesWithoutZoneController.$inject = ['PackagesService', '$state', '$http', 'UrlHelper', '$scope', '$filter', 'DELIVERY_TERM','ProfileService'];

    function AdminPackagesWithoutZoneController(PackagesService, $state, $http, UrlHelper, $scope, $filter, DELIVERY_TERM, ProfileService) {
        var vm = this;
        angular.extend(vm, {
        ...
        function getPackages(page, take, desc, isScheduled, userId, search, status, sortColumn, city) {
            ...
            return PackagesService.getPageWithoutZone(page, take, desc, isScheduled, userId, search, status, sortColumn, city,searchArrived);
        ...
        function showPackage(obj) {
            var photo = obj.avatar;
                ...
                .then(function(response) {
                    ...
                    $state.go('admin.viewPackage', { obj: response.data , previusUrl: "admin.packageswithoutzone"});
                })
                .catch(function(error) {
                    console.log(error);
                });
